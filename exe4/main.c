...
SemaphoreHandle_t xSemaphore_r2;

void btn_callback(uint gpio, uint32_t events) {
    if (gpio == BTN_PIN_R && events == 0x4) { // fall edge
        xSemaphoreGiveFromISR(xSemaphore_r, 0);
    } else if (gpio == BTN_PIN_G && events ==0x4){
        xSemaphoreGiveFromISR(xSemaphore_r2,0);
    }
}
...
void btn_2_task(void *p){
    ...
    int delay = 0;
    while (true){
        if (xSemaphoreTake(xSemaphore_r2, pdMS_TO_TICKS(500)) == pdTRUE) {
            if (delay < 1000){
                delay += 100;
            } else{
                delay = 100;
...
int main() {
    ...
    xSemaphore_r2 = xSemaphoreCreateBinary();
